...
const flashcardsListViewDefinitionUp = knex =>
    knex
        .select(
            'f.*',
...
const flashcardsListViewDefinitionDown = knex =>
    knex
        .select(
            'f.*',
            knex.raw('b.id as book_id'),
...
const up = async knex => {
    ...
    await knex.raw(`CREATE VIEW ${flashcardListViewName} AS ${flashcardsListViewDefinitionUp(knex)}`)
...
const down = async knex => {
    ...
    await knex.raw(`CREATE VIEW ${flashcardListViewName} AS ${flashcardsListViewDefinitionDown(knex)}`)
