...
exports.up = async knex => changeToBigInteger(knex)
exports.down = async knex => changeToSerial4(knex)
...
const changeToBigInteger = async knex => {
    ...
    await knex.schema.alterTable(tableName, table => {
        table.bigInteger(columnName).defaultTo(null).alter()
    })
...
const changeToSerial4 = async knex => {
    await knex.raw(`DROP VIEW IF EXISTS ${flashcardListViewName}`)
    await knex.raw(`DROP VIEW IF EXISTS ${studentFlashcardStudyViewName}`)
    await knex.schema.alterTable(tableName, table => {
        table.increments(columnName, { primaryKey: false }).alter()
    })
