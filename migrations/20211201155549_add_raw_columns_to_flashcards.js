...
const addColumn = (knex) => (
  knex.schema.table(tableName, table => {
    table.text('raw_question')
    table.text('raw_explanation')
  })
)
...
const dropColumn = async (knex) => {
  ...
  await knex.schema.table(tableName, table => {
    table.dropColumn('raw_question')
    table.dropColumn('raw_explanation')
  })

  await knex.raw(`CREATE VIEW ${viewName} AS ${flashcardsListViewDefinition(knex)}`)
}
