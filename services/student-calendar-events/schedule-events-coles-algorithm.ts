...
const fillCalendarWithTasks = (numTasks: number, prioridays: number[], daysCount: number[], numTests: number, schedule, debug = false): EventScheduleWorkDay[] => {
  ...
  for (let i = 0; i <= 7; i++) {
    if (prioridays[i] > 0 && prioridays[i] < 6) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (priority > 0) {
      ...
      residual = (remainingTasks - taskDensity[priority] * remainingDays) / daysCount[priority]
      ...
      remainingTasks -= taskDensity[priority] * daysCount[priority]
      remainingDays -= daysCount[priority]
    }
  }
    ...
    if (schedule[i].onBreak === false) {
      ...
      if (floatingDensity[dayOfWeekIndex] > 0) {
        ...
        for (let j = 0; j < taskAmount; j++) {
          if (schedule[i].dayOfWeek === priorities[6] && testNumber <= numTests) {
...
export const scheduleCalendarEvents = (config: Config, debug = false) => {
  ...
  for (let i = 0; i < schedule.length; i++) {
    ...
    if (prioridays[schedule[i].dayOfWeek - 1] > 0 && prioridays[schedule[i].dayOfWeek - 1] < 7) {
  ...
  if (numDays > 60) {
    ...
    for (let i = 1; i <= numBreaks; i++) {
      ...
      while (schedule[startBreak] && schedule[startBreak].dayOfWeek !== 1) {
        startBreak += 1
      }
  ...
  return fillCalendarWithTasks(numTasks, prioridays, daysCount, numTests, schedule, debug)
}
