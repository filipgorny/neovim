...
import { ExamType } from '../../src/models'
import mapP from '../../utils/function/mapp'
import { collectionToJson } from '../../utils/model/collection-to-json'
import { createExamIntroPage } from '../../src/modules/exam-intro-pages/exam-intro-page-service'

const RECORDS_PER_BATCH = 100

const log = batchNumber => console.log(`-> filling missing intro pages for all exam types; batch ${batchNumber}`)
...
const processType = async examType => {
  ...
  const introPages = examType.introPages
  ...
  await mapP(
    // @ts-ignore
    async order => createExamIntroPage('', {}, examType.id, order)
  )(missingOrder)
}
...
const nextBatch = async (batchNumber, step) => {
  return R.pipeWith(R.andThen)([
    async () => fetch(ExamType)({}, ['introPages'], buildPaginationData(batchNumber, step)),
    R.prop('data'),
    collectionToJson,
  ])(true)
}
...
(
  async (): Promise<void> => {
    ...
    console.log('Filling missing intro pages for all exam types.')
    process.exit(0)
  }
)()
